...
import useFetchLaboratorios from "./hooks/useFetchLaboratorios";
import { useLaboratoriosHandlers } from "./hooks/useLaboratoriosHandlers";

const ViewLaboratorios: React.FC = () => {
    const [verArchivados, setVerArchivados] = useState(false);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editingLaboratorio, setEditingLaboratorio] = useState<Laboratorio | null>(null);

    const { data, setData, loading } = useFetchLaboratorios(verArchivados);
    ...
    const columns: ColumnDef<Laboratorio>[] = [
        ...
        cell: ({ row }) => {
            ...
            <button
                ...
                onClick={() => handleRestore(laboratorio.id)}
            >
                Restaurar
            </button>
            ) : (
    ...
    const fields = [
        { name: "nombre", label: "Nombre", type: "text", placeholder: "Ingrese el nombre del laboratorio", maxLength: 30, required: true, pattern: "^[A-Za-záéíóúÁÉÍÓÚñÑ0-9\\s]+$",
            autoFocus: true
        },
        ...
        <div className="p-4 flex-1 bg-white dark:bg-gray-800">
            ...
            >
                ...
                <div className="flex-1 text-center font-bold text-black dark:text-white text-3xl">
                    {verArchivados ? "Laboratorios Archivados" : "Laboratorios Activos"}
                </div>
            </div>
            ...
            <p className="text-center text-gray-500 mt-4">No hay laboratorios archivadas.</p>
            )}
...
export default ViewLaboratorios;
